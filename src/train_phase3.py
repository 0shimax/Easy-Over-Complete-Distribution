...
def train(args, mnasnet, cvae, regressor, cvae_optimizer, regressor_optimizer, data_loader,
          lambda_reg=.1, lambda_c=.1):
    train_loss = 0
    for epoch in range(args.epoch):
        for batch_idx, (anchor, positive, negative, label, neg_label) in enumerate(data_loader):
            ...
            with torch.no_grad():
                anc_cl, pos_cl, neg_cl = regressor(anchor, positive, negative)
            loss = lambda_reg*regressor.loss(anc_cl, pos_cl, neg_cl, label, label, neg_label)

            with torch.no_grad():
                anc1hot = one_hot(anc_cl.argmax(dim=1, keepdim=True), args.n_class)
                pos1hot = one_hot(pos_cl.argmax(dim=1, keepdim=True), args.n_class)
                neg1hot = one_hot(neg_cl.argmax(dim=1, keepdim=True), args.n_class)
            recon_anchor = cvae(anchor, anc1hot)
            recon_positive = cvae(positive, pos1hot)
            recon_negative = cvae(negative, neg1hot)
            loss = loss + lambda_c*cvae.loss(recon_anchor, anchor.detach())
            loss = loss + lambda_c*cvae.loss(recon_positive, positive.detach())
            loss = loss + lambda_c*cvae.loss(recon_negative, negative.detach())

            anc_cl, pos_cl, neg_cl = regressor(recon_anchor, recon_positive, recon_negative)
            loss = loss + regressor.loss(anc_cl, pos_cl, neg_cl, label, label, neg_label)
            
            loss.backward()
            train_loss += loss.item()

            torch.nn.utils.clip_grad_norm_(cvae.parameters(), .1)
            torch.nn.utils.clip_grad_norm_(regressor.parameters(), .1)
            cvae_optimizer.step()
    ...
    torch.save(cvae.state_dict(),
               '{}/cvae_phase3.pth'.format(args.out_dir))
    torch.save(regressor.state_dict(),
               '{}/regressor_phase3.pth'.format(args.out_dir))
...
def main(args):
    ...
    train(args, mnasnet, cvae, regressor, cvae_optimizer, regressor_optimizer, data_loader)
...
if __name__=="__main__":
    ...
    parser.add_argument('--cvae-resume-model', default='./result/wbc/model_phase1.pth', help='path to trained model')
